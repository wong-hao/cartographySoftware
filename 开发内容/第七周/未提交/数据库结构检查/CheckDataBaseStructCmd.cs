    ...
    public class CheckDataBaseStructCmd : SMGI.Common.SMGICommand
    {
        ...
        public static string CompareFieldsStruct(IFeatureClass objFC, IFeatureClass templateFC)
        {
            ...
            for (var i = 0; i < templateFields.FieldCount; i++)
            {
                ...
                if (!fd.Name.Equals(objFD.Name, StringComparison.OrdinalIgnoreCase))
                {
                    info += "名称名称不匹配（不考虑大小写）；";
                }
                ...
                {
                    //info += "类型不匹配，其中模板字段类型为" + fd.Type + "，而目标字段类型为" + objFD.Type + "；";
                    info += "类型不匹配；";
                }
                ...
                {
                    //info += "长度不匹配，其中模板字段长度为" + fd.Length + "，而目标字段长度为" + objFD.Length + "；";
                    info += "长度不匹配；";
                }
