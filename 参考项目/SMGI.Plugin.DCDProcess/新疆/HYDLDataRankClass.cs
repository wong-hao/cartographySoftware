...
{
    //河流选取等级赋初值（字段名：GRADE2）
    //参考配置表：规则配置.mdb\河流选取等级赋初值
    //2022.3.2 修改 张怀相
    //功能修改：
    //  1、执行后加载对话框 GRADEListFrm
    //  2、对话框里设置 是否 采用 特定等级，默认采用特定等级
    //  3、从下拉列表里选择特定等级，程序只对符合该等级的河流进行GRADE2赋值
    public class HYDLDataRankClass : SMGI.Common.SMGICommand
    {
        ...
        {  
            ...
            //设置等级选取对话框
            GradeListFrm frm = new GradeListFrm("河流选取等级GRADE2", ranks);
