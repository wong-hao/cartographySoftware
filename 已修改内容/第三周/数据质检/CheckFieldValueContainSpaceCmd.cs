...
namespace SMGI.Plugin.CollaborativeWorkWithAccount
{
    ...
    /// 字段含有空格检查
    /// </summary>
    public class CheckFieldValueContainSpaceCmd : SMGICommand
    {
        ...
        {
            var frm = new CheckLayerWithFieldsForm(m_Application, true);
        ...
        public static string DoCheck(string resultTxtFileName, Dictionary<IFeatureClass, List<string>> fc2FieldList, WaitOperation wo = null)
        {
            ...
            try
            {
                ...
                foreach (var kv in fc2FieldList)
                {
                    ...
                    if (wo != null)
                        wo.SetText(string.Format("正在对要素类【{0}】进行字段值空格检查......", fc.AliasName));
                    ...
                    while ((fe = feCursor.NextFeature()) != null)
                    {
                        foreach (var fd in fieldList)
                        {
                            ...
                            string fdValue = fe.get_Value(fdIndex).ToString();
                            if (fdValue.Contains(" ") || fdValue.Contains("　"))
                            {
                    ...
                    if (oid2FieldList.Count > 0)
                    {
                        ...
                        //写入结果文件
                        resultSW.WriteLine(string.Format("要素类【{0}】的字段值空格检查结果【{1}】：", fc.AliasName, oid2FieldList.Count));
