...
namespace SMGI.Plugin.CollaborativeWorkWithAccount
{
    ...
    public class cmdUpdateRecord : SMGI.Common.SMGICommand
    {
        ...
        public static bool EnableAttributeBrushToolForm { get; set; }//使用属性刷时，是否进行弹窗
        ...
        public void UApplication_OnDeleteFeature(ESRI.ArcGIS.Geodatabase.IObject obj)
        {
            ...
            if (GApplication.NoDelete || !EnableUpdate)//未开启更新，不修改状态（制图中心要求）
            {
                return;
            }
        ...
        public void UApplication_OnChangeFeature(ESRI.ArcGIS.Geodatabase.IObject obj)
        {
           ...
           // fe.set_Value(fe.Fields.FindField("VERS"), Vers);
            ...
            if (EnableChangeFeatureForm)//是否弹弹窗
            {
                ...
                para.Add(fe.Class.AliasName);
                para.Add(fe.OID.ToSafeString());
                para.Add(fe.get_Value(fe.Fields.FindField(ServerDataInitializeCommand.CollabSTACOD)).ToSafeString());
                OnChangeFeatureForm frm = new OnChangeFeatureForm(m_Application, para);
                if (frm.ShowDialog() == DialogResult.OK)
                {
                    ...
                    checkFieldIndex = fe.Fields.FindField("check");
                    if (checkFieldIndex == -1)
                        checkFieldIndex = fe.Fields.FindField("check_");
                    if (checkFieldIndex != -1)
                        fe.set_Value(checkFieldIndex, "");
                }
            }
        }
