        ...
        public ResultMessage SaveResult(string dir, bool bAutoOver = false, bool toGdb=false)
        {
            ...
            try
            {
                if (_errorInfoList.Count > 0)
                {
                    ...
                    var sf = shpdir + "\\端点离线检查_" + _lyrName;
                    ...
                    {
                        shpWriter.addErrorGeometry(tp.Item2, new Dictionary<string, string> { { "图层名", _lyrName }, { "编号", tp.Item1 }, { "检查项", "点离线检查" } });
                    }
        ...
        ResultMessage Analysis(IFeatureClass fcls, IEnvelope env, double disValue,WaitOperation wo = null)
        {
            ...
            {                
                ...
                else
                {
                    ...
                    string txt = origLineFID.ToString();
