    ...
    public class CheckFCFieldValueRelationshipHardCodedCmd : SMGICommand
    {
       ...
       private static string TYPE = "TYPE";
        ...
        public override void OnClick()
        {
            ...
            using (var wo = m_Application.SetBusy())
            {
                ...
                if (outputFileNameList.Count > 0)
                {
                    ...
                    {
                        IFeatureClass errFC = OpenSHPFile(outputFileNameInList);

                        fcNameArray = fcNameArray + fullPath + "\\" + errFC.AliasName + ";";
                    }
                }
                ...
                try
                {
                    string outPutFileName = OutputSetup.GetDir() + string.Format("\\属性逻辑关系硬编码检查.shp");
        ...
        public static IFeatureClass OpenSHPFile(string shpFileName)
        {
            ...
            FeatureClassToFeatureClass featureClassToFeature = new FeatureClassToFeatureClass();
            featureClassToFeature.in_features = System.IO.Path.GetDirectoryName(shpFileName) + "\\" + fc.AliasName + ".shp";
            featureClassToFeature.out_path = DCDHelper.GetAppDataPath() + "\\" + "MyWorkspace.gdb";
            featureClassToFeature.out_name = fc.AliasName;

            try
            {
                Helper.ExecuteGPTool(geoprocessor, featureClassToFeature, null);
            }
            catch (Exception ex)
            {
