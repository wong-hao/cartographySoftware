...
namespace SMGI.Plugin.DCDProcess
{
    ...
    /// 线重叠检查
    /// </summary>
    public class CheckNoLineOverlapCmd : SMGICommand
    {
        ...
        public static string DoCheck(string resultSHPFileName, List<IFeatureClass> fcList, WaitOperation wo = null)
        {
            ...
            try
            {
                ...
                foreach (var fc in fcList)
                {
                    ...
                    if (wo != null)
                        wo.SetText(string.Format("正在对要素类【{0}】进行线重叠检查......", fc.AliasName));

                    //核查该图层的重叠点
                    Dictionary<IPolyline, KeyValuePair<int, int>> errList = CheckHelper.LineNoOverlap(fc, qf);
                    if (errList.Count > 0)
                    {
                        if (resultFile == null)
                        {
                        ...
                        foreach (var kv in errList)
                        {
                            ...
                            fieldName2FieldValue.Add("检查项", "线重叠检查");
