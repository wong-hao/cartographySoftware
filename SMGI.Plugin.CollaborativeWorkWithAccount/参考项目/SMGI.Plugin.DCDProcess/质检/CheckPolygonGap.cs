    ...
    public class CheckPolygonGap
    {
        ...
        public string DoCheck(string resultSHPFileName, IFeatureClass fc, WaitOperation wo = null)
        {
            ...
            try
            {
                ...
                IFeatureClass temp_FC = VEGAHelper.CreateFeatureClassStructToWorkspace(fws, fc, fc.AliasName + "_temp");
                string filter = "";
                if (fc.FindField(cmdUpdateRecord.CollabVERSION) != -1)
                    filter = string.Format("({0} <> {1} or {2} is null)", cmdUpdateRecord.CollabVERSION, cmdUpdateRecord.DeleteState, cmdUpdateRecord.CollabVERSION);
