...
namespace SMGI.Plugin.DCDProcess
{
    ...
    /// 面化简交互式工具
    /// </summary>
    public class PolylineGeneralizeTool : SMGITool
    {
        ...
        public override void OnClick() 
        {
            ...
            {
                //弹框，设置成员参数
                var frm = new GeneralizeForm(esriGeometryType.esriGeometryPolyline, m_Application.MapControl.Map.ReferenceScale, false, false);
                frm.Text = "线化简";
        ...
        public override void OnMouseDown(int button, int shift, int x, int y)
        {
            ...
            try
            {
                using (var wo = m_Application.SetBusy())
                {
                    ...
                    var bsuccess = PolylineGeneralizeCmd.Generalize(_inputLayer.FeatureClass, filterText, _simplyAlgorithm, _simplifyTolerance, _enableSmooth, _smoothAlgorithm, _smoothTolerance, true);
                    if (bsuccess)
                    {
                        //刷新
                        m_Application.ActiveView.PartialRefresh(esriViewDrawPhase.esriViewGeoSelection, null, null);
        ...
        public override void OnKeyUp(int keyCode, int shift)
        {
            switch (keyCode)
            {
                case 32:
                    //弹框，设置成员参数
                    var frm = new GeneralizeForm(esriGeometryType.esriGeometryPolyline, _inputLayer, _refScale, _simplyAlgorithm, _simplifyTolerance, _enableSmooth, _smoothAlgorithm, _smoothTolerance, false);
                    frm.Text = "线化简";
                    if (frm.ShowDialog() != System.Windows.Forms.DialogResult.OK)
                    {
                        if (_inputLayer == null)
